        ...
        <div class="col-lg-12 mb-4 order-0">
            <div class="card">
                <div class="card-header bg-primary">
                    <h4 class="text-title text-white">Data Status</h4>
                </div>
                ...
                <div class="card-footer mt-3">
                    <button type="button" class="btn btn-sm btn-success" id="formStatus"><?php echo $title;?></button>
...
<script type="text/javascript">
    ...
    $('#formStatus').on('click', function() {
        var id = `<?php echo $id;?>`;
        let api = id == 0 ? '/status/add' : '/status/update'
    ...
    $(document).ready(function() {
        ...
        if(id != 0) {
            $.ajax({
                url: url_api + '/status/'+ id,
                type: 'get',
                success: function(res) {
                    var data = res.data
                    data.forEach(function(items, index) {
                        $('#name').val(items.statusName)
                        $('#code').val(items.statusCode)
                    })
                }
            })
        }
